...
func ExportWithAttempts(exp EventExporter, eEv any, key string) (err error) {
	if exp.Cfg().FailedPostsDir != utils.MetaNone {
		defer func() {
			if err != nil {
				utils.Logger.Debug(fmt.Sprintf("### ExportWithAttempts - before AddFailedPost - err: %v", err))
				utils.Logger.Debug(fmt.Sprintf("### ExportWithAttempts - before AddFailedPost - exp.Cfg().ExportPath: %v", exp.Cfg().ExportPath))
				utils.Logger.Debug(fmt.Sprintf("### ExportWithAttempts - before AddFailedPost - exp.Cfg().Type: %v", exp.Cfg().Type))
				utils.Logger.Debug(fmt.Sprintf("### ExportWithAttempts - before AddFailedPost - utils.ToJSON(eEv): %v", utils.ToJSON(eEv)))
				utils.Logger.Debug(fmt.Sprintf("### ExportWithAttempts - before AddFailedPost - utils.ToJSON(exp.Cfg().Opts): %v", utils.ToJSON(exp.Cfg().Opts)))
